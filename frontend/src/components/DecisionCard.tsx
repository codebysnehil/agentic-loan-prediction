...
export const DecisionCard: React.FC<DecisionCardProps> = ({ result, summary }) => {
  const vc = VERDICT_CONFIG[result.verdict];

  const metrics: [string, string][] = [
    ["Financial Score", `${result.financial_score}/100`],
    ["Debt-to-Income",  `${(result.dti_ratio * 100).toFixed(0)}%`],
    ["Interest Rate",   `${result.recommended_rate}% p.a.`],
  ];
    ...
    <>
      <div style={{ background: vc.bg, border: `1.5px solid ${vc.border}`, borderRadius: 12, padding: "1.25rem", marginBottom: "1rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <div style={{ width: 44, height: 44, borderRadius: "50%", background: vc.border, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 20, fontWeight: 700 }}>
            {vc.icon}
          </div>
          <div>
            <p style={{ fontSize: 12, color: vc.text, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.05em" }}>Eligibility Decision</p>
            <h2 style={{ fontSize: 22, fontWeight: 700, color: vc.text }}>{result.verdict}</h2>
          </div>
        </div>
        ...
        {result.recommendations.length > 0 && (
          <div>
            <p style={{ fontSize: 13, fontWeight: 500, color: vc.text, marginBottom: 8 }}>Recommendations</p>
